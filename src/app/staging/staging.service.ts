...
export class StagingService {
    ...
    /**
        * Called any time user signs in
        * works with firebase authorization
        * saves returned user data to real-time database
     */
	saveUserData(imageUrl, displayName, uid): Promise<void> {
		// validate?
		if (!displayName) {
		...
		// Data to say to real-time database people location
		const updateData = {
		...
		return this.database.object(`/people/${uid}`).update(updateData);
		//
	}
